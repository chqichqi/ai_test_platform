...
def test_conversion(token: str, test_case_id: str):
    ...
    if response.status_code == 200:
        ...
        print(f"  Web UI test case ID: {result.get('web_ui_test_case_id')}")
        print(f"  Test script: {result.get('test_script', {})}")
        
        if result.get('data'):
            data = result['data']
            print(f"  Data web_ui_test_cases: {data.get('web_ui_test_cases')}")
            print(f"  Data element_selectors: {data.get('element_selectors')}")
            print(f"  Data test_script: {data.get('test_script')}")
